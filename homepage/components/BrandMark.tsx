...
const SIZE_CLASS = {
  compact: "h-8 w-8",
  default: "h-9 w-9",
  mobile: "h-28 w-28 sm:h-36 sm:w-36",
...
export function BrandMark({ size = "default", className = "" }: BrandMarkProps) {
  const isHero = size === "hero" || size === "mobile";
  ...
  return (
    ...
    >
      {isHero ? (
        <span
          className="pointer-events-none absolute inset-[-18%] -z-10 opacity-50"
          style={{
            background:
              "radial-gradient(circle at 55% 45%, color-mix(in srgb, var(--accent-primary) 35%, transparent), transparent 58%)",
            filter: "blur(40px)",
          }}
        />
      ) : null}
