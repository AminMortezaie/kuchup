...
func fetchJoblet(ctx context.Context, client getter, req Request) ([]Job, error) {
	...
	order := []string{}
	var mu sync.Mutex
	tasks := make([]func(context.Context, getter) error, 0, len(jobletQueries))
	for _, query := range jobletQueries {
		query := query
		tasks = append(tasks, func(ctx context.Context, g getter) error {
			api := "https://joblet.ai/api/search?" + url.Values{
				"q":        {query},
				"location": {"Remote"},
				"page":     {"1"},
			}.Encode()
			res, err := getURL(ctx, g, api, headers)
			if err != nil {
				return err
			}
			if err := res.requireOK(); err != nil {
	...
	if pool, ok := client.(*HTTPPool); ok {
		if err := pool.RunGroup(ctx, tasks); err != nil {
			return nil, err
		}
	} else {
		for _, task := range tasks {
			if err := task(ctx, client); err != nil {
				return nil, err
			}
		}
	}
