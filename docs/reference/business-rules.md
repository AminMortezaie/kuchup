...
5a. **Application states (panel Applications)** — **Apply** iff `(pinned OR looking_to_apply) AND NOT applied AND NOT rejected AND NOT not_for_me`. **Applied** iff `applied AND NOT rejected AND NOT not_for_me`. **Rejected** iff `rejected AND NOT not_for_me`. Any not-for-me reason removes the role from all three tabs. Panel `/applications` tabs and MCP `list_application_queue` / `in_application_queue` share the Apply rule. Counts use `GET /api/applications/counts`; each state list is independently paginated. On the board, pin toggles stick-to-top and `looking_to_apply` together; public save and `/api/jobs/looking-to-apply` can still set looking-to-apply without pinning.
