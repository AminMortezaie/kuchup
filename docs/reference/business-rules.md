...
5a. **Active application queue** — A position is in the active queue iff `(pinned OR looking_to_apply) AND NOT applied`. Panel `/applications` and MCP `list_application_queue` / `in_application_queue` share this rule. Pinning and Want to apply are independent reasons for membership; neither requires the other.
