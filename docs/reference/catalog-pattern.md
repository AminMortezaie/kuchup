...
How this panel separates **shared job data** from **per-user state**.
...
- [catalog-seed-test-failure.md](../archive/catalog-seed-test-failure.md) — why tests must reset catalog, not assume an empty DB
- [board.md](board.md) — sort and pagination on top of flattened catalog rows
- [board-read-model-proposal.md](../proposals/board-read-model-proposal.md) — proposal to persist per-user board projection for fast reads (not implemented)
