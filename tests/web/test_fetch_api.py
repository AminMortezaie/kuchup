...
def test_country_fetch_reaps_orphan_running_row(v2_auth_client, seeded_catalog_v2, monkeypatch):
    ...
    monkeypatch.setattr(
        "relocation_jobs.web.routes.fetch.start_country_fetch",
        fake_start,
    )
...
def test_fetch_status_includes_review_jobs_after_company_fetch(v2_auth_client, monkeypatch):
    ...
    assert status["review_jobs"]["included"][0]["title"] == "Backend Engineer"
    assert len(status["review_jobs"]["filtered"]) == 1
    assert status["review_jobs"]["filtered"][0]["filter_reason"] == "title keyword"
